...
class CheckYandexAnalytics
{
    ...
    public function check(Command $command)
    {
        ...
        foreach ($period as $date) {
            ...
            foreach ($companies as $company) {
                ...
                $roistatAnalytic = new RoistatAnalytic;
                $roistatAnalytic->timestamps = false;
                $roistatAnalytic->roistat_company_config_id = $roistatConfig->id;
                $roistatAnalytic->visitCount = $convertedAnalyticData['visitCount'] ?? 0;
                $roistatAnalytic->visits2leads = $convertedAnalyticData['visits2leads'] ?? 0;
                $roistatAnalytic->leadCount = $convertedAnalyticData['leadCount'] ?? 0;
                $roistatAnalytic->visitsCost = $convertedAnalyticData['visitsCost'] ?? 0;
                $roistatAnalytic->costPerClick = $convertedAnalyticData['costPerClick'] ?? 0;
                $roistatAnalytic->costPerLead = $convertedAnalyticData['costPerLead'] ?? 0;
                $roistatAnalytic->for_date = $date->format('Y-m-d');
                $roistatAnalytic->created_at = $date->format('Y-m-d 01:00:00');
                $roistatAnalytic->updated_at = $date->format('Y-m-d 01:00:00');
                $roistatAnalytic->save();
    ...
    private function deleteAnalytic()
    {
        ...
        RoistatAnalytic::where('created_at', '>=', $date_from)
